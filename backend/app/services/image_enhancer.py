...
def call_gemini_extract_rooms_from_base64(
    b64: str, model: str = "gemini-2.5-flash"
) -> List[Dict[str, Any]]:
    if genai is None:
        raise RuntimeError("google-generativeai SDK is not installed or failed to import")
    ...
    prompt_text = (
        """Reconstruct and enhance the provided floor plan image into a clean, high-resolution, professional architectural drawing. Remove all gibberish text, distortions, and unclear markings. Sharpen all walls, symbols, lines, and labels.

Identify each room based on furniture and layout, and correctly label them (e.g., Master Bedroom, Bedroom, Kitchen, Living Room, Dining, Bathroom, Toilet, Study, Garage, Utility, Balcony, etc.).

For every room:
- Add a clear label with: Room Name, Length × Width (in feet), and Total Area (sq ft).
- Place a small marker in the room center showing its (x, y) coordinate in a 0–100 grid.
- Use clean architectural fonts and accurate dimension lines.

Add a 0–100 coordinate grid around the drawing for reference.

Ensure the final image looks like an AutoCAD/Revit-quality architectural floor plan: crisp lines, uniform scaling, readable text, and no noise or artifacts. """
    )
    ...
    text_out = response.text if hasattr(response, 'text') else None
